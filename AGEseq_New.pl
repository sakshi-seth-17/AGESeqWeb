...

{
	...
	sub handle_request {
		...
		my $readDirectory = "//data//AGEseq//$timestamp"."_reads";
		mkdir $readDirectory or die "Failed to create directory: $!";
		...
		foreach my $filename (keys %$alldata) {
			...
			open(my $fh, '>', "$readDirectory//$filename") or die "Could not open file '$filename' $!";
		...
		my $current_directory = Cwd::getcwd();  # Get the current working directory
		my $targetsFilepath = "$current_directory//$timestamp"."targets.txt";  # Construct the file path
		...
		ageSeq($readDirectory, $targetsFilepath, $timestamp."Output.txt");
		# Send the response
...
#	
#	if ($path_info eq $endpoint) {
#	
#		my $timestamp = localtime->strftime('%Y%m%d%H%M%S');
#		
#		# Get parameters from the request
#		my $dat_dir = $cgi->param('input');
#		my $file_design = $cgi->param('target');
#		my $final_out = $timestamp.'_'.$cgi->param('output');
#		
#		
#		ageSeq($dat_dir, $file_design, $final_out);
#		
#		# Generate JSON response
#		my $response = { message => "Successful!" };
#	
#		# Send the response
#        print "HTTP/1.0 200 OK\r\n";
#        print "Content-Type: application/json\r\n\r\n";
...
#	
#	}else {
#		# Print error response for invalid endpoint
...
#$server->run();
#$server->run($hostname, $port, \&handle_request);
...
#my $final_out   =  $timestamp.'_'.$ARGV[2];
#
#
##my ($dat_dir, $file_design, $final_out) = @_;  # Retrieve the arguments
#
